---
title: '**Identifying heterogeneous subgroups of systemic connective tissue diseases by applying a joint dimension reduction and clustering approach to immunomarkers : a retrospective study** <br/>- data wrangling'
author: |
...
```{r Quick view for reference data, eval=FALSE, echo=FALSE}
### A quick view for reference data
str(exam_table)
...
```{r Summarise Error ICD, warning=FALSE}
map(icd_conversion, 
...
```{r Standardised ICD code and diagnosis dataset subseting}
substitute_index <-
  map(diag_table_name, 
      ~ raw_dataset[[.x]] %>%
...
```{r Disease group identification}
...
```{r Eligible monoCTD case identification}
...
```{r lab and exam data preprocess}
...
```{r Combine lab and exam and report data}
...
```{r Create a result data container}
...
```{r Clean raw results and merge reference information, message=FALSE, warning=FALSE, cache=TRUE}
...
```{r Determination of stauts for multiple test results}
simul_classify_item <- 
...
```{r within-seven-day exam result merging, eval=TRUE}
...
```{r Prepare for final lab and report data (within-seven-day exam results), eval=TRUE}
...
```{r Calculate time span between the first exam and diagnosis date}
...
```{r Visualise time span between the first exam and diagnosis date, message=FALSE, results=FALSE, warning=FALSE}
...
<br/> 
![Figure. Rheumatoid arthritis ](./Data_Wrangling_files/first_diag_exam_span/first_diag_exam_span_plot_Rheumatoid arthritis.jpeg  ) 
![Figure. Dermatopolymyositis ](./Data_Wrangling_files/first_diag_exam_span/first_diag_exam_span_plot_Sjogren's syndrome.jpeg) 
![Figure. Systemic lupus erythematosus ](./Data_Wrangling_files/first_diag_exam_span/first_diag_exam_span_plot_Systemic lupus erythematosus.jpeg)

## Calculate interval between first diagnosis and each exam records, and subset the data set by the specified intervals

```{r Subset data set by specified intervals}
...
```{r Quick view for ready-for-analysis data, echo=FALSE}
copy(monoCTD_ready_analysis_dataset) %>% 
  { .[,.SD,.SDcols = -str_subset(names(.),"IDCODE|ID|LABNO")] } %>% 
str(.)
...
![</br>**Figure 1**</br>](./Heterogeneity-in-SCTD-paper_files/Figure 1. Eligibility and the number of study subjects.jpeg)
**Figure 1. Eligibility and the number of study subjects.**
