...
### Identify eligible cases with a single SAD over the observed courses and exclude UCTD cases
```{r Identify eligible cases with a single SAD over the observed courses and exclude UCTD cases}
...
### Identify cases with multiple SADs over the observed courses and exclude UCTD cases
```{r Proportion of multiple SADs, echo=TRUE, warning=FALSE, message=TRUE}
number_of_multi_CTD <- 
  pluck(CustomGrepGroup,"groupedDT") %>% 
    .[ID %in% map_df(eligible_case_list,~ .x[,.(ID)])$ID, ] %>% 
    .[,Group := factor(Group,levels = disease_ICD_table$Group)] %>% 
    ...
    # To check details for SAD types
    # .[,unique(.SD), .SDcols = c("ID", "Group")]
    # To check how many were affected by multiple SADs
    .[, uniqueN(ID)]

numberi_of_all_wo_UCTD <- 
  pluck(CustomGrepGroup,"groupedDT") %>% 
  .[ID %in% map_df(eligible_case_list,~ .x[,.(ID)])$ID, ] %>% 
  .[,Group := factor(Group,levels = disease_ICD_table$Group)] %>% 
  split(.,by = "ID") %>% 
...
print(
  paste0("Cases of multiple SADs account for ",
         round(number_of_multi_CTD/numberi_of_all_wo_UCTD*100,1),
         "% out of all SADs without for UCTD")
  )
```
<br/>
