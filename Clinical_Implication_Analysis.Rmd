---
title: '**Identifying heterogeneous subgroups of systemic autoimmune diseases by applying a joint dimension reduction and clustering approach to immunomarkers : a retrospective study** <br/>- clinical implication analysis'
author: |
...
```{r GitHub packages, warning=FALSE, message=FALSE}
...
From [Cluster_Analysis.md](https://github.com/DHLab-TSENG/Heterogeneity-in-SAD-paper/blob/main/Cluster_Analysis.md)
```{r Import clustering result datasets, results='hide', message=FALSE}
...
```{r Convert ICD-code into comorbidities, cache=TRUE, message=FALSE, results='hide'}
...
```{r Prepare plot data for displaying positive rate of each ICD category by cluster}
...
## ICD categories
```{r Visualize positive rate of each ICD category by cluster, message=FALSE, results='hide'}
paletteLength <- 20
mid_point <- 65
min_value <- 0
max_value <- 100
...
## Combine heatmaps of CCS and ICD category into a uniframe - Figure 4[^1]
```{r Combine heatmaps of CCS and ICD category into a uniframe, echo=TRUE, message=FALSE, results='hide'}
...
```{r Datasets for profiling the proportion differences, echo=TRUE, message=FALSE, results='hide'}
library(gtsummary)
library(gt)
...
comorbidity_proportion_diff_test_data <- 
  map(list(comorbidity_CCS_proportion_test_data,
      ...
      ~ .x[, .SD, .SDcols = -c("ID")] %>% 
        ...
        .[, Group := factor(Group, levels = PC_data[,levels(Group)])]
      )
```
<br/>

## Conduct statistical test for the proportion differences in the clusters - between six groups
```{r tests for the proportion differences between six groups, echo=TRUE, message=FALSE, results='asis'}
fisher_test_fun <- 
...
```{r display statistical test results, eval=FALSE, echo=FALSE}
knitr::knit_print(between_six_groups_tbl)

map2(between_six_groups_tbl,
     list("CCS", "ICD"),
     ~ .x %>% 
       # as_raw_html() %>% 
       gtsave(.,
              filename = paste0(.y,"_proportion_differences_between_clusters.html"),
              path = "./Clinical_Implication_Analysis_files/")
     )
```

## Conduct statistical test for the proportions differences in the clusters - between each two groups
```{r tests for the proportion differences between each two groups, echo=TRUE, message=FALSE, results='hide'}
set.seed(1111)
...
between_each_two_groups_comp <- 
  expand.grid(Group = PC_data[,levels(Group)],
              Category = c("CCS", "ICD"),
              Comp_pair = comp_pair,
        ...
        map(Test_data,
            ~ .x %>% 
              ...
              #               `upper.95.CI` = .$`conf.high` %>% round(., 3) ) } %>%
                  ...
                  ~ BarnardTest(x = as.matrix(.x),
                                method = "boschloo") # “csm”, “csm approximate”, “z-pooled”, “z-unpooled”, “boschloo”, “santner and snell” %>%
                    broom::tidy(.) ) %>%
              rbindlist(.,idcol = "variable") %>%
              { data.table( Variable = .$variable,
                            `p.value` = .$`p.value` %>% round(., 3) ) } %>%
...
```{r query for test results of the proportion differences between each two groups, eval=FALSE, echo=FALSE}
copy(between_each_two_groups_comp) %>% 
  # .[Group %in% "Sjogren's syndrome"] %>%
  # .[Group %in% "Rheumatoid arthritis"] %>%
  .[Group %in% "Systemic lupus erythematosus"] %>%
  
  .[Category %in% "CCS",] %>%
  # .[Category %in% "ICD"] %>%
  
  # .[Comp_pair %in% "Cluster 1 vs. 2",] %>% #
  .[Comp_pair %in% "Cluster 1 vs. 3",] %>% # 
  # .[Comp_pair %in% "Cluster 1 vs. 4",] %>% #
  # .[Comp_pair %in% "Cluster 1 vs. 5",] %>% #
  # .[Comp_pair %in% "Cluster 1 vs. 6",] %>% #
  # .[Comp_pair %in% "Cluster 2 vs. 3",] %>% # 
  # .[Comp_pair %in% "Cluster 2 vs. 4",] %>% #
  # .[Comp_pair %in% "Cluster 2 vs. 5",] %>% #
  # .[Comp_pair %in% "Cluster 2 vs. 6",] %>% #
  # .[Comp_pair %in% "Cluster 3 vs. 4",] %>% #
  # .[Comp_pair %in% "Cluster 3 vs. 5",] %>% #
  # .[Comp_pair %in% "Cluster 3 vs. 6",] %>% #
  # .[Comp_pair %in% "Cluster 4 vs. 5",] %>% #
  # .[Comp_pair %in% "Cluster 4 vs. 6",] %>% #
  # .[Comp_pair %in% "Cluster 5 vs. 6",] %>% #
